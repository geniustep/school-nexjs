...
export function canShowEditAcademicTerm(
  ...
  if (actions) {
    // After this OR fails, TS has narrowed edit_dates away from `true`,
    // so a second `edit_dates !== true` check is redundant and illegal.
    if (actions.edit === true || actions.edit_dates === true) return true;
    if (actions.edit === false) return false;
  }
