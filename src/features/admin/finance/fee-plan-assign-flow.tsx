...
export function FeePlanAssignFlow({ plan }: { plan: FeePlan }) {
  ...
  if (!validation.canAssign) {
    return (
      <div className="fee-plan-assign-page">
        <FeePlanAssignBreadcrumb plan={plan} />
        <header className="fee-plan-assign-page__header">
          <h1>{t('admin.finance.assignFlow.pageTitle')}</h1>
        </header>
        <div className="fee-plan-assign-layout">
          <main className="fee-plan-assign-layout__main">
          ...
          <aside className="fee-plan-assign-layout__aside">
            <FeePlanAssignSourceCard
              plan={plan}
              yearLabel={yearLabel}
  ...
  return (
    <div className="fee-plan-assign-page">
      ...
      <header className="fee-plan-assign-page__header">
        <h1>{t('admin.finance.assignFlow.pageTitle')}</h1>
        <p className="fee-plan-assign-page__desc">{t('admin.finance.assignFlow.pageDescEligible')}</p>
      </header>

      <div className="fee-plan-assign-layout">
        <main className="fee-plan-assign-layout__main">
          {step !== 'result' ? (
            <div className="fee-plan-assign-workspace card">
              <FeePlanAssignStepper steps={stepperSteps} current={step} />
              <div className="fee-plan-assign-workspace__body">
                {step === 'students' ? (
                  <FeePlanAssignStudentsStep
                    plan={plan}
                    ...
                    onSelectedIdsChange={(_ids, students) => setSelectedStudents(students)}
                    onNext={() => setStep('preview')}
                  />
                ) : null}

                {step === 'preview' ? (
                  <div className="fee-plan-assign-preview">
                    <h2 className="fee-plan-assign-preview__title">{t('admin.finance.assignFlow.previewTitle')}</h2>
                    ...
                    <label className="fee-plan-assign-preview__date">
                    ...
                    <div className="fee-plan-assign-flow__desktop-table student-360-table-wrap">
                      <table className="data fee-plan-assign-flow__preview-table">
                        <thead>
                          <tr>
          ...
          {step === 'result' && assignResults ? (
            <section className="card fee-plan-assign-flow__section fee-plan-assign-result">
