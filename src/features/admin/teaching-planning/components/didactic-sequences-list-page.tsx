 ...
 *
 * Didactic Sequences list. A Didactic Sequence is a lesson/unit plan of session
 * TEMPLATES — it is NOT a Jathatha (daily prep sheet) and NOT a scheduled session.
 */
...
import { canManageDidacticSequences } from '@/lib/permissions/teaching-planning';
...
export function DidacticSequencesListPage() {
  const t = useT();
  ...
  const state = useAdminResource(endpoints.admin.didacticSequences, params);
  const rows = useMemo(
    () => filterDidacticSequencesClient(normalizeDidacticSequences(state.data), search),
    [state.data, search],
  );
  const pg = state.meta?.pagination;
  const canCreate = canManageDidacticSequences(user);
  ...
  const columns: Column<DidacticSequenceSummary>[] = useMemo(
    () => [
      {
        key: 'name',
        header: t('admin.teachingPlanning.sequences.columns.name'),
        render: (row) => (
      ...
      {
        key: 'sessions',
        header: t('admin.teachingPlanning.sequences.columns.expectedSessions'),
        render: (row) => (
          <Badge tone="blue">
            <bdi dir="ltr">{row.expected_session_count}</bdi>
          </Badge>
        ),
      },
      {
        key: 'templates',
        header: t('admin.teachingPlanning.sequences.columns.templates'),
        render: (row) => <bdi dir="ltr">{row.session_template_count}</bdi>,
      },
      {
        key: 'state',
      ...
      },
      {
        ...
        render: (row) => (
          <div className="tp-list__row-actions" onClick={(event) => event.stopPropagation()}>
            <Link
              href={`/admin/teaching-planning/sequences/${row.id}`}
  ...
  const listEmptyState =
    emptyVariant === 'noMatch' ? (
      <EmptyState
        icon="🔍"
        title={t('admin.teachingPlanning.sequences.list.noMatch.title')}
        description={t('admin.teachingPlanning.sequences.list.noMatch.description')}
        action={
      ...
      <EmptyState
        icon="🧩"
        title={t('admin.teachingPlanning.sequences.list.noData.title')}
        description={t('admin.teachingPlanning.sequences.list.noData.description')}
        action={
          canCreate ? (
            <button
              type="button"
            ...
            >
              + {t('admin.teachingPlanning.sequences.create.open')}
            </button>
          ) : undefined
        }
      />
  ...
  return (
    <RequireTeachingPlanningAccess>
      <div className="admin-workspace tp-list-page">
        ...
        <PageHeader
          title={t('admin.teachingPlanning.sequences.title')}
          subtitle={
            pg
              ? t('admin.teachingPlanning.sequences.subtitleWithCount', { total: pg.total })
              : t('admin.teachingPlanning.sequences.subtitle')
          }
          actions={
            canCreate ? (
        ...
        {state.fetching ? (
          <p className="tp-list__fetching-hint" aria-live="polite">
            {t('admin.teachingPlanning.sequences.refetching')}
          </p>
        ) : null}
                ...
                <>
                  <div className="tp-list__table">
                    <DataTable
                      ...
                      rows={data}
                      rowKey={(row) => row.id}
                      onRowClick={(row) => router.push(`/admin/teaching-planning/sequences/${row.id}`)}
                    />
                  </div>
                  {pg ? (
                    <Pagination
        ...
        <DidacticSequenceEditorDialog
          ...
          onSaved={(item) => {
            setCreateOpen(false);
            router.push(`/admin/teaching-planning/sequences/${item.id}`);
          }}
        />
      </div>
    </RequireTeachingPlanningAccess>
