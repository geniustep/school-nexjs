  ...
  return (
    <LibraryModal title="إعارة سريعة" onClose={onClose}>
      <form className="form-stack" onSubmit={search}>
        ...
        <p className="library-form-note">عند وجود تطابق واحد متاح للإعارة سينتقل مباشرة إلى اختيار المستعير.</p>
        {error ? <p className="library-form-error" role="alert">{error}</p> : null}

        {!loading && searched && results.length === 0 && !error ? (
          <div className="state state--compact">
            <div className="state__title">لم نجد نسخة مطابقة</div>
            <div className="state__desc">تحقق من الباركود أو رقم الجرد ثم أعد المحاولة.</div>
          </div>
        ) : null}
