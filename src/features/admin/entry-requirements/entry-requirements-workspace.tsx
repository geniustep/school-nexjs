...
function approvedOfferings(rows: TeachingOfferingChoice[]): TeachingOfferingChoice[] {
  return rows.filter((row) => row.reference.id && row.reference.status === 'approved');
}
...
export function AdminEntryRequirementsWorkspace() {
  ...
  const [resolutionOfferings,setResolutionOfferings] = useState<TeachingOfferingChoice[]>([]);
  ...
  async function openList(id:number){
    setError('');
    const r=await api.get<RequirementList>(entryRequirementEndpoints.admin.list(id));
    if(!r.success)setError(r.error.message);else setSelected(r.data);
  }

  async function mutate(path:string,body:unknown={}){
    setError('');setNotice('');
    const r=await api.post<RequirementList|Record<string,unknown>>(path,body);
    if(!r.success){setError(r.error.message);return false;}
  ...
  async function createList(e:FormEvent){
    ...
    const r=await api.post<RequirementList>(entryRequirementEndpoints.admin.lists,{academic_year_id:activeAcademicYearId,level_id:Number(newLevel),class_id:newClass?Number(newClass):undefined,name:newName});
    if(!r.success){setError(r.error.message);return;}
    setNewOpen(false);setNotice('تم إنشاء المسودة.');await loadLists();await openList(r.data.id);
  }

  useEffect(()=>{
    if(itemType!=='textbook'||!selected||!itemSubject){setOfferings([]);setOfferingId('');return;}
    void api.get<TeachingOfferingChoice[]>(entryRequirementEndpoints.admin.teachingOfferings,{academic_year_id:selected.academic_year_id,level_id:selected.level_id,subject_id:Number(itemSubject),track_id:selected.track_id??undefined,page:1,page_size:80}).then(r=>{if(r.success)setOfferings(approvedOfferings(r.data));});
  },[itemSubject,itemType,selected]);

  useEffect(()=>{
    if(!selected||!resolutionItemId||!resolutionSubject){setResolutionOfferings([]);setResolutionOfferingId('');return;}
    let cancelled=false;
    setResolutionLoading(true);
    void api.get<TeachingOfferingChoice[]>(entryRequirementEndpoints.admin.teachingOfferings,{academic_year_id:selected.academic_year_id,level_id:selected.level_id,subject_id:Number(resolutionSubject),track_id:selected.track_id??undefined,page:1,page_size:80}).then(r=>{
      if(cancelled)return;
      setResolutionLoading(false);
      if(r.success)setResolutionOfferings(approvedOfferings(r.data));else{setResolutionOfferings([]);setError(r.error.message);}
    });
    return()=>{cancelled=true;};
  },[resolutionItemId,resolutionSubject,selected]);

  useEffect(()=>{
    setResolutionItemId(null);
    setResolutionSubject('');
    setResolutionOfferingId('');
    setResolutionOfferings([]);
  },[selected?.id]);

  async function addItem(e:FormEvent){
    e.preventDefault();if(!selected)return;
    const body:Record<string,unknown>={item_type:itemType,quantity:Number(itemQty)||1,importance:itemImportance,provision_source:itemProvision,reusable_allowed:itemReusable||undefined};
    if(itemType==='textbook'){
      if(!offeringId){setError('اختر المقرر المعتمد أولًا.');return;}
      body.teaching_offering_id=Number(offeringId);
      const off=offerings.find(o=>String(o.id)===offeringId);
      body.name=off?.reference.title||'كتاب مقرر';
    }else{
      if(!itemName.trim()){setError('أدخل اسم العنصر.');return;}
      body.name=itemName.trim();
      if(itemSubject)body.subject_id=Number(itemSubject);
    }
    const r=await api.post<RequirementItem>(entryRequirementEndpoints.admin.items(selected.id),body);
    if(!r.success){setError(r.error.message);return;}
    setItemName('');setOfferingId('');setNotice('تمت إضافة العنصر.');await openList(selected.id);
  }

  function startResolution(item:RequirementItem){
    setError('');setNotice('');
    setResolutionItemId(item.id);
    setResolutionSubject(item.subject_id?String(item.subject_id):'');
    setResolutionOfferingId('');
    setResolutionOfferings([]);
  }
  ...
  async function resolveTextbook(){
    ...
    setResolutionLoading(true);setError('');setNotice('');
    const r=await api.patch<RequirementItem>(
    ...
    setResolutionItemId(null);setResolutionSubject('');setResolutionOfferingId('');setResolutionOfferings([]);
    await openList(selected.id);
  }

  async function deleteItem(item:RequirementItem){
    if(!selected)return;
    const r=await api.delete<{deleted:boolean}>(entryRequirementEndpoints.admin.item(selected.id,item.id));
    if(!r.success)setError(r.error.message);else await openList(selected.id);
  }

  async function publish(){
    if(!selected)return;
    setError('');
    const r=await api.post<{warnings:Array<{message:string}>;published_revision:RequirementList}>(entryRequirementEndpoints.admin.publish(selected.id),{});
    if(!r.success){const details=r.error.details as {blockers?:Array<{message:string}>;warnings?:Array<{message:string}>}|undefined;const blockers=details?.blockers?.map(x=>x.message).join(' · ');setError(blockers||r.error.message);return;}
    setNotice(r.data.warnings?.length?`تم النشر مع ${r.data.warnings.length} تنبيه.`:'تم نشر اللائحة.');await loadLists();await openList(selected.id);
  }
  ...
  const unresolvedCount=selected?.items?.filter(item=>item.item_type==='textbook'&&item.needs_resolution&&!item.teaching_offering_id).length??0;
  ...
  return <div className={styles.workspace}>
    ...
    {selected?<section className={styles.card}>
      ...
      <EntryRequirementsOperations list={selected} canManage={canManage} canPublish={canPublish} onChanged={async()=>{await loadLists();await openList(selected.id);}}/>
      <h3 className={styles.sectionTitle}>عناصر اللائحة</h3>
      {unresolvedCount>0&&editable&&canManage?<div className={styles.notice}>هناك {unresolvedCount} كتابًا مقررًا يحتاج ربطه بالمقرر المعتمد قبل النشر. اختر المادة ثم المقرر صراحة؛ لن يجري رقيم مطابقة تخمينية.</div>:null}
      <div className={styles.list}>{selected.items?.map(item=>{
        const unresolved=item.item_type==='textbook'&&item.needs_resolution&&!item.teaching_offering_id;
        const resolving=resolutionItemId===item.id;
        return <div key={item.id} className={styles.row}><div className={styles.rowMain}><strong>{item.name} {item.quantity!==1?`× ${item.quantity}`:''}</strong><span className={styles.tiny}>{requirementItemTypeLabel(item.item_type)}{item.subject?` · ${item.subject}`:''}{item.provision_source==='school'?' · توفره المدرسة':''}{item.reusable?' · قابل لإعادة الاستعمال':''}</span>{unresolved?<span className={styles.badge}>يحتاج ربطًا بالمقرر</span>:item.item_type==='textbook'&&item.teaching_offering_id?<span className={styles.badge}>مرتبط بالمقرر</span>:null}{unresolved&&canManage&&editable?<button type="button" className="btn btn--ghost btn--sm" onClick={()=>startResolution(item)}>ربط بالمقرر</button>:null}{resolving?<div className={styles.card}><div className={styles.formGrid}><label className={styles.field}>المادة<select className="select" value={resolutionSubject} onChange={e=>{setResolutionSubject(e.target.value);setResolutionOfferingId('');}}><option value="">اختر المادة</option>{subjects.map(subject=><option key={subject.id} value={subject.id}>{subject.name??`#${subject.id}`}</option>)}</select></label><label className={`${styles.field} ${styles.fieldFull}`}>المقرر المعتمد<select className="select" value={resolutionOfferingId} disabled={!resolutionSubject||resolutionLoading} onChange={e=>setResolutionOfferingId(e.target.value)}><option value="">{resolutionLoading?'جارٍ تحميل المقررات…':'اختر المقرر'}</option>{resolutionOfferings.map(off=><option key={off.id} value={off.id}>{off.subject} — {off.reference.title}{off.reference.edition?` — ${off.reference.edition}`:''}{off.language?` — ${off.language}`:''}</option>)}</select>{resolutionSubject&&!resolutionLoading&&resolutionOfferings.length===0?<span className={styles.muted}>لا يوجد مقرر بمرجع معتمد لهذه المادة في السنة والمستوى المحددين.</span>:null}</label></div><div className={styles.actions}><button type="button" className="btn btn--primary" disabled={!resolutionOfferingId||resolutionLoading} onClick={()=>void resolveTextbook()}>تأكيد الربط</button><button type="button" className="btn btn--ghost" onClick={()=>{setResolutionItemId(null);setResolutionSubject('');setResolutionOfferingId('');setResolutionOfferings([]);}}>إلغاء</button></div></div>:null}</div>{canManage&&editable?<button className="btn btn--ghost btn--sm" onClick={()=>void deleteItem(item)}>حذف</button>:null}</div>;
      })}</div>
      {canManage&&editable?<form className={styles.card} onSubmit={addItem}><h3 className={styles.sectionTitle}>إضافة عنصر</h3><div className={styles.formGrid}><label className={styles.field}>النوع<select className="select" value={itemType} onChange={e=>setItemType(e.target.value as RequirementItemType)}>{ITEM_TYPES.map(t=><option key={t} value={t}>{requirementItemTypeLabel(t)}</option>)}</select></label><label className={styles.field}>الكمية<input className="input" type="number" min="0.1" step="0.1" value={itemQty} onChange={e=>setItemQty(e.target.value)}/></label>{itemType==='textbook'?<><label className={styles.field}>المادة<select className="select" required value={itemSubject} onChange={e=>setItemSubject(e.target.value)}><option value="">اختر المادة</option>{subjects.map(s=><option key={s.id} value={s.id}>{s.name??`#${s.id}`}</option>)}</select></label><label className={`${styles.field} ${styles.fieldFull}`}>المقرر المعتمد<select className="select" required value={offeringId} onChange={e=>setOfferingId(e.target.value)}><option value="">اختر المقرر</option>{offerings.map(o=><option key={o.id} value={o.id}>{o.subject} — {o.reference.title}{o.reference.edition?` — ${o.reference.edition}`:''}</option>)}</select>{itemSubject&&offerings.length===0?<span className={styles.muted}>لا يوجد مقرر معتمد لهذه المادة بعد.</span>:null}</label></>:<label className={`${styles.field} ${styles.fieldFull}`}>اسم العنصر<input className="input" required value={itemName} onChange={e=>setItemName(e.target.value)} placeholder="مثال: دفتر 100 ورقة"/></label>}<label className={styles.field}>الأهمية<select className="select" value={itemImportance} onChange={e=>setItemImportance(e.target.value as 'required'|'optional')}><option value="required">إلزامي</option><option value="optional">اختياري</option></select></label><label className={styles.field}>من يوفره؟<select className="select" value={itemProvision} onChange={e=>setItemProvision(e.target.value as 'family'|'school')}><option value="family">الأسرة</option><option value="school">المدرسة</option></select></label><label className={styles.field}>إعادة الاستعمال<select className="select" value={itemReusable} onChange={e=>setItemReusable(e.target.value as 'yes'|'no'|'')}><option value="">غير محدد</option><option value="yes">يمكن إعادة استعماله</option><option value="no">لا</option></select></label></div><button className="btn btn--primary">إضافة</button></form>:<div className={styles.notice}>اللائحة المنشورة أو قيد المراجعة لا تعدّل مباشرة. أنشئ نسخة محدثة لإجراء التغييرات.</div>}
      <div className={styles.notice}>إذا أضاف المتصفح التاريخ أو عنوان الصفحة عند الحفظ PDF، عطّل خيار «Headers and footers / En-têtes et pieds de page» في نافذة الطباعة؛ المتصفح وحده يتحكم في هذه الترويسة.</div>
    </section>:null}
  </div>;
}
