...
describe('TeacherLifecycleDialogs', () => {
  it('requires archive reason before calling API', async () => {
    ...
    expect(
      screen.getByText('admin.teacherDomain.errors.archiveReasonRequired'),
    ).toBeTruthy();
  });

  it('requires terminate reason and supports end date', async () => {
    terminateTeacher.mockResolvedValue({
      success: true,
      data: { id: 1, name: 'أستاذ', code: 'T1', status: 'terminated' },
      meta: {},
    });
    ...
    fireEvent.change(within(dialog).getByRole('textbox'), {
      target: { value: 'سبب الإنهاء' },
    });
    fireEvent.change(within(dialog).getByDisplayValue(''), {
      target: { value: '2026-07-20' },
    });
    fireEvent.click(
      within(dialog).getByRole('button', {
        name: 'admin.teacherDomain.lifecycle.terminate',
      }),
    );

    await waitFor(() => {
      expect(terminateTeacher).toHaveBeenCalledWith(1, {
        reason: 'سبب الإنهاء',
        employment_end_date: '2026-07-20',
      });
      expect(onSuccess).toHaveBeenCalled();
    });
  });

  it('runs reactivate confirmation without local transition matrix', async () => {
    reactivateTeacher.mockResolvedValue({
      success: true,
      data: { id: 1, name: 'أستاذ', code: 'T1', status: 'active' },
      meta: {},
    });
    render(
      <TeacherLifecycleDialogs
        teacher={{ id: 1, name: 'أستاذ', code: 'T1', status: 'archived' }}
        action="reactivate"
        onClose={vi.fn()}
        onSuccess={vi.fn()}
      />,
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'admin.teacherDomain.lifecycle.reactivate' }),
    );
    await waitFor(() => {
      expect(reactivateTeacher).toHaveBeenCalledWith(1);
    });
  });
});
