...
export function StudentsFilteredRosterExport({ filters, levels, filterDescription }: {
  ...
  function print() {
    ...
    printWindow.document.write(`<!doctype html><html lang="${esc(locale)}" dir="${locale === 'ar' ? 'rtl' : 'ltr'}"><head><meta charset="utf-8"><title>${esc(labels.title)}</title><style>@page{size:A4 landscape;margin:11mm}body{font:12px Arial,Tahoma;color:#111827}.header{text-align:center;margin-bottom:10px}h1{margin:0 0 6px;font-size:20px}.filter{font-size:12px}table{width:100%;border-collapse:collapse;table-layout:auto}thead{display:table-header-group}th,td{border:1px solid #4b5563;padding:6px 7px;vertical-align:middle}th{background:#f3f4f6}td:nth-child(2){font-weight:600}tr{height:30px;page-break-inside:avoid}.footer{margin-top:7px;font-weight:700}</style></head><body><header class="header"><h1>${esc(labels.title)}</h1><div class="filter"><b>${esc(labels.filter)}:</b> ${esc(filterDescription)}</div></header><table><thead><tr>${headers}</tr></thead><tbody>${table}</tbody></table><div class="footer">${esc(labels.count)}: ${sorted.length}</div></body></html>`);
  ...
  async function excel() {
    try {
      ...
      const sheet = workbook.addWorksheet(labels.title.slice(0, 31), { views: [{ rightToLeft: locale === 'ar', state: 'frozen', ySplit: 4 }] });
      sheet.columns = columns.map((key) => ({ width: key === 'name' ? 34 : key === 'notes' ? 32 : 18 }));
