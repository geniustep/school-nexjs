...
function JourneyStepCard({ step, displayNumber }: { step: GuidedStep; displayNumber: number }) {
  ...
  const isLocked = !step.available;

  return (
    <article
      className={cn(
        'academic-journey-step',
        `academic-journey-step--${tone}`,
        isLocked && 'academic-journey-step--locked',
        isDone && 'academic-journey-step--done',
        step.state === 'in_progress' && 'academic-journey-step--current',
      )}
    >
      <div className="academic-journey-step__main">
        ...
        <div className="academic-journey-step__copy">
          ...
          {step.available ? (
            ...
            <button
              type="button"
              className="btn btn--ghost btn--sm academic-journey-step__cta"
              disabled
              aria-disabled
            >
              {step.state === 'locked'
  ...
  return (
    <section className="academic-journey" aria-labelledby="guided-flow-title">
      ...
      <ol className="academic-journey__list">
        {core.map((step, index) => (
          <li key={step.id} className="academic-journey__item">
      ...
      {(staff || review) && (
        <div className="academic-journey__side">
          {staff && (
