...
export function ParentEntryRequirementsWorkspace() {
  ...
  return <div className={styles.workspace}>
    <PageHeader title="تجهيزات الدخول المدرسي" subtitle="الكتب والأدوات المطلوبة لأبنائك مع قائمة متابعة بسيطة." actions={child ? <button className="btn btn--ghost no-print" onClick={()=>window.print()}>طباعة اللائحة</button> : undefined} />
    ...
    {child ? <>
      {family && family.children.length>1 ? <button className="btn btn--ghost no-print" onClick={()=>setChild(null)}>العودة إلى الأبناء</button> : null}
