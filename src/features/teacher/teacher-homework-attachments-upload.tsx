...
import {
  HOMEWORK_ATTACHMENT_MAX_FILES,
  buildHomeworkAttachmentsFormData,
  validateHomeworkUploadFiles,
  type HomeworkUploadRejectReason,
} from '@/lib/attachments/homework-upload';
import type { HomeworkAttachmentsUploadData } from '@/types/attachment';

interface TeacherHomeworkAttachmentsUploadProps {
  homeworkId: number;
  existingCount: number;
  onUploaded: () => void;
}

function rejectMessage(t: (k: string) => string, reason: HomeworkUploadRejectReason): string {
  switch (reason) {
...
export function TeacherHomeworkAttachmentsUpload({
  homeworkId,
  existingCount,
  onUploaded,
}: TeacherHomeworkAttachmentsUploadProps) {
  const t = useT();
  ...
  const slotsLeft = HOMEWORK_ATTACHMENT_MAX_FILES - existingCount;
  ...
  function handleFilesChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const validation = validateHomeworkUploadFiles(picked, existingCount);
  ...
  async function upload() {
    const validation = validateHomeworkUploadFiles(pending, existingCount);
    if (!validation.ok && validation.reason) {
      toast.error(rejectMessage(t, validation.reason));
      return;
    }
    ...
    setUploading(true);
    const res = await api.uploadForm<HomeworkAttachmentsUploadData>(
      endpoints.teacher.homeworkAttachments(homeworkId),
      buildHomeworkAttachmentsFormData(pending),
    );
    setUploading(false);

    if (res.success) {
      toast.success(t('teacher.attachmentsUploadSuccess'));
      setPending([]);
      setOpen(false);
      onUploaded();
    } else {
