...
import styles from './prepare-campaign.module.css';
...
export default function ParentActivationCampaignPreparePage() {
  ...
  return (
    <div className={`page-shell ${styles.campaignWorkspace}`}>
      <PageHeader
        title={t('admin.parentActivation.prepare')}
        subtitle={t('admin.parentActivation.stepOneDescription')}
        actions={
      ...
      <section className={styles.prepareHero}>
        <div className={styles.heroCopy}>
          <Badge tone="blue">{t('admin.parentActivation.previewOnlyTitle')}</Badge>
          <h1>{t('admin.parentActivation.prepare')}</h1>
          <p>{t('admin.parentActivation.previewOnlyDescription')}</p>
        </div>
        <div className={styles.stageRail} aria-label={t('admin.parentActivation.title')}>
          <div className={styles.stageItem} data-state="active">
            <span>1</span>
            <strong>{t('admin.parentActivation.prepare')}</strong>
          </div>
          <div className={styles.stageDivider} aria-hidden="true" />
          <div className={styles.stageItem} data-state={campaign ? 'active' : 'pending'}>
            <span>2</span>
            <strong>{ui.operationsTitle}</strong>
          </div>
          <div className={styles.stageDivider} aria-hidden="true" />
          <div className={styles.stageItem} data-state={campaign && selectedForDispatch > 0 ? 'active' : 'pending'}>
            <span>3</span>
            <strong>{dispatchCopy.dispatchStepTitle}</strong>
          </div>
        </div>
      </section>
      ...
      <Card className={styles.setupCard}>
        <div className={styles.setupHeading}>
          ...
          <div>
            ...
            <p>{t('admin.parentActivation.stepOneDescription')}</p>
          </div>
        ...
        <form onSubmit={prepare} className={styles.previewForm}>
          <label className="field">
            ...
            <input
              ...
              maxLength={160}
              disabled={busy}
              placeholder={t('admin.parentActivation.nameLabel')}
            />
          </label>
          <button type="submit" className={`btn btn--primary ${styles.prepareAction}`} disabled={busy}>
